...
export class ProfessorSelectionService {
  ...
  async selectRandomProfessors(count: number = 3): Promise<string[]> {
    ...
    if (!result.success || !result.data.users.length) {
      throw new UnprocessableEntityException('Nenhum professor encontrado no sistema');
    }

    const professors = result.data.users;

    if (professors.length < count) {
      throw new UnprocessableEntityException(`Não há professores suficientes no sistema. Encontrados: ${professors.length}, necessários: ${count}`);
    }
