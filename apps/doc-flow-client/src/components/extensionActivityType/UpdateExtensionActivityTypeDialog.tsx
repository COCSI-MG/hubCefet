import { ExtensionActivityType, extensionActivityTypeService, UpsertExtensionActivityType } from "@/api/services/extension-activity-type.service"
...
interface UpdateExtensionActivityTypeDialogProps {
  pagination: Pagination;
  fetchExtensionActivityTypes: (pagination: PaginationArgs) => Promise<void>;
  item: ExtensionActivityType | null;
  setItem: React.Dispatch<React.SetStateAction<ExtensionActivityType | null>>;
  isModalOpen: boolean;
...
type updateExtensionActivityType = z.infer<typeof extensionActivityTypeSchema>


export function UpdateExtensionActivityTypeDialog({ pagination, fetchExtensionActivityTypes, item, isModalOpen, setIsModalOpen, setItem }: UpdateExtensionActivityTypeDialogProps) {
  const { register, handleSubmit, reset, formState: { errors } } = useForm<updateExtensionActivityType>({
  ...
  const handleUpdate = async (formData: UpsertExtensionActivityType) => {
    if (!item) return

    try {
      await extensionActivityTypeService.update(item.id, formData)
      ...
      toast.success("Tipo atividade de extensão atualizada com sucesso!")
    } catch (err) {
      ...
      toast.error("Nao foi possivel atualizar um tipo de atividade de extensão")
    }
  }
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-lg shadow-lg z-50 w-[38rem]">
          <Dialog.Title className="text-xl font-bold">
            Atualizar Tipo de Atividade de Extensão
          </Dialog.Title>
          <Dialog.Description className="text-gray-600">
            Atualize os dados de um tipo de atividade de extensão
          </Dialog.Description>
