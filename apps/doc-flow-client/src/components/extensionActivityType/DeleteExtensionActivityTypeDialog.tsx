...
interface DeleteExtensionActivityTypeDialogProps {
...
export function DeleteExtensionActivityTypeDialog({ fetchExtensionActivityTypes, pagination, item, isModalOpen, setIsModalOpen, setItem }: DeleteExtensionActivityTypeDialogProps) {

  const handleDelete = async () => {
    if (!item) return

    try {
      await extensionActivityTypeService.remove(item.id)
      ...
      toast.success("Tipo atividade de extensão excluída com sucesso!")
    } catch (err) {
      ...
      toast.error("Nao foi possivel deletar um tipo de atividade de extensão")
    }
  }
